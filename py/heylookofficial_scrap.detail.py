...
cur = conn.cursor()
cur.execute("select a.href, a.name from heylookofficial_master a left join heylookofficial_images b on a.name=b.name where b.name is null order by cast(replace(replace(a.rating,'(',''),')','') as integer) limit 10")
...
for line in reader:
    ...
    empty = ""
    try:
        elems = driver.find_element_by_xpath('//div[@data-testid="txtPDPWarningEmptyStock"]')
        print("Stock Empty, Continue")
        empty = elems.text
    except:
